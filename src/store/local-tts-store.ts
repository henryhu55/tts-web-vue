...
export const useLocalTTSStore = Pinia.defineStore("localTTSStore", {
  state: () => {
    return {
      ...
      config: {
        ...
        baseUrl:
          store.get("localTTS.baseUrl") ?? DEFAULT_LOCAL_TTS_CONFIG.baseUrl,
        defaultVoice:
          store.get("localTTS.defaultVoice") ??
          DEFAULT_LOCAL_TTS_CONFIG.defaultVoice,
        defaultLanguage:
          store.get("localTTS.defaultLanguage") ??
          DEFAULT_LOCAL_TTS_CONFIG.defaultLanguage,
      },
      // 服务器状态
      serverStatus: {
        connected: false,
  ...
  actions: {
    ...
    async getAudioStream(
      ...
      try {
        ...
        const params = {
          ...
          pitch: pitch,
          style: style || "Default",
          "style-degree": style && style !== "Default" ? "4.0" : undefined,
        };

        console.log("TTS请求参数:", {
          url: `${this.config.baseUrl}/api/v1/free-tts-stream`,
          params,
        });
    ...
    async getAndPlayAudio(
      ...
      const url = await this.getAudioStream(
        text,
        voice,
        language,
        format,
