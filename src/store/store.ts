    ...
    setSSMLValue(text = "", forceUpdate = false) {
      ...
      const ssmlContent = `<speak xmlns="http://www.w3.org/2001/10/synthesis" xmlns:mstts="https://www.w3.org/2001/mstts" xmlns:emo="http://www.w3.org/2009/10/emotionml" version="1.0" xml:lang="en-US">
        <voice name="${voice}">
            ...
            }${role != "Default" ? ' role="' + role + '"' : ""} styledegree="2">
                ${silenceConfig}${text}
            </mstts:express-as>
        </voice>
      </speak>`;
    ...
    async start() {
      ...
      // 批量转换
        ...
        try {
          ...
          // 获取待转换的文件列表
          const pendingFiles = this.tableData.filter(
          ...
          const inputs = await Promise.all(
            pendingFiles.map(async (file: any) => {
              try {
                // 设置文件状态为处理中
                ...
                if (
                  ...
                  let intensityValue = "";
                  if (this.formConfig.intensity === "weak")
                    intensityValue = "0.5";
                  else if (this.formConfig.intensity === "strong")
                    intensityValue = "1.5";
                  else if (this.formConfig.intensity === "extraStrong")
                    intensityValue = "2";
                  else intensityValue = this.formConfig.intensity;
                ...
                let volumeAttr = "";
                if (
                  this.formConfig.volume &&
                  this.formConfig.volume !== "default"
                ) {
                  ...
                  volumeAttr = ` volume="${
                    volumeMapping[this.formConfig.volume] ||
                    this.formConfig.volume
                  }"`;
                }
          ...
          const checkStatus = async () => {
            ...
            try {
              ...
              if (status.properties) {
                const total =
                  status.properties.succeededAudioCount +
                  status.properties.failedAudioCount;
                ...
                if (status.properties.failedAudioCount > 0) {
                  console.warn(
                    `当前有 ${status.properties.failedAudioCount} 个文件转换失败`
                  );
                }
              }
              ...
              switch (status.status) {
                ...
                case "Succeeded":
                  if (status.outputs?.result) {
                    ...
                    ElMessage({
                      message: `批量转换完成，成功: ${
                        status.properties?.succeededAudioCount || 0
                      } 个，失败: ${
                        status.properties?.failedAudioCount || 0
                      } 个`,
                      type: "success",
                      duration: 3000,
                    });
